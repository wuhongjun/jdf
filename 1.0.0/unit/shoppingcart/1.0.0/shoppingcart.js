	...
	var mcart={
		...
		init:function(){
			...
			if(document.getElementById("settleup")){
				...
				this.el.find('.dorpdown-layer').html('加载中，请稍候...');
			}
		...
		TPL_List: {
			wrap: '<div class="spacer"></div><div id="settleup-content"><div class="smt"><h4 class="fl">最新加入的商品</h4></div><div class="smc"></div><div class="smb ar">共<b>${Num}</b>件商品　共计<strong>￥ ${TotalPromotionPrice.toFixed(2)}</strong><br><a href="http://cart.jd.com/cart/cart.html?r=${+new Date}" title="去购物车结算" id="btn-payforgoods">去购物车结算</a></div></div>',
