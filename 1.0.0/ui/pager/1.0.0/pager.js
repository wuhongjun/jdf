...
*  - `prevText`{String}  '上一页'  上一页文案
*  - `nextText` {String} '下一页'  下一页文案
	...
	$.ui.define('pager', {
		...
		rander:function(){
			...
			if (opts.currentPage>1){
				html +=  '<a rel="'+(opts.currentPage-1)+'" class="'+opts.prevClass+'" href="'+pageHref+'">上一页</a>';
			}
			...
			//下一页
			if (opts.currentPage<opts.totalPage){
				html +=  '<a rel="'+(opts.currentPage+1)+'" class="'+opts.nextClass+'" href="'+pageHref+'">下一页</a>';
			}
        ...
        */
		...
		bind:function(){
			...
			this.el.find('.'+opts.prevClass).bind('click',function(event){
				me.update($(this).attr('rel'));
			})

			this.el.find('.'+opts.nextClass).bind('click',function(event){
				 me.update($(this).attr('rel'));
			})
		},
		init:function(){
