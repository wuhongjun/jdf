...
*
* 2014-11-5 15:56:00 by chenxiaochun
* 主要更改了组件的运行方式，1.0.0版本是页面onload之后会在body中添加多个tips。
* 此版本是只有当用户点击相应的元素之后，才会在body中去添加一个tips。
*
	...
	$.ui.define('tips', {
		...
		init:function(){
			...
			$(document).click(function(e){
				...
				/**当点击文档中任意位置时，需要判断四个条件来决定是否隐藏当前的tips
					...
					5、当前是否有可见的tips
				*/
				if(!$(target).is(me.el) && !$(target).parents().is(me.el) && !$(target).is('.ui-tips') && !$(target).parents('.ui-tips').is('.ui-tips') && $('.ui-tips:visible').length){
					me.hide();
				}
			});
			...
			if (opts.type == 'hover') {
				...
				this.tips.bind('mouseenter', function(event) {
					mouseenterFn();
				});

				this.tips.bind('mouseleave', function(event) {
					mouseleaveFn();
				});
			}
		...
		show:function(){
			if (this.options.type == 'fixed') {
				$(this.tips).appendTo(this.el);
			}else {
				$(this.tips).appendTo('body');
			}

			//在el上添加一个class样式
			this.el.addClass(this.options.currClass);
		},
		/**
		 * @隐藏
		 * @method hide
		...
		hide:function(){
			...
			//移除el上的class样式
			this.el.removeClass(this.options.currClass);;

			//调用onclose回调
			if(this.options.onClose){this.options.onClose.call(null, this.el, this.tips)};
		},
		/**
		 * @隐藏所有tips
		 * @method hide
		...
		setInit:function(el){
			var tipsContent = this.options.source || el.attr('data-tips');
			if (tipsContent) {
				this.show();
				...
				this.options.callback.call(this.tips.find('.ui-tips-main'), this.el, this.tips);
			}
		},
		bind:function(){
			var me = this;
			...
			me.tips.find(me.options.closeName).bind('click', function(){
				me.hide();
			});
		...
		create:function(){
			...
			var templete = [
				'<div class="'+opts.tipsClass+' ui-tips-top" style="z-index:'+zIndex+'" id="uis-tips-'+this.guid+'">',
